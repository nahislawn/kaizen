...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener {
    ...
    public static final String FORCE_DISCONNECT_VPN = "de.blinkt.openvpn.FORCE_DISCONNECT_VPN";
    ...
    PendingIntent getContentIntent() {
        try {
            if (mNotificationActivityClass != null) {
                ...
                return PendingIntent.getActivity(this, 0, intent,  getFlagIntent());
            }
        } catch (Exception e) {
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel,
        ...
        if (status == LEVEL_WAITING_FOR_USER_INPUT) {
            PendingIntent pIntent = PendingIntent.getActivity(this, 0, intent,  getFlagIntent());
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        ...
        PendingIntent disconnectPendingIntent = PendingIntent.getActivity(this, 0, disconnectVPN,  getFlagIntent());
...
//        }
    ...
    PendingIntent getGraphPendingIntent() {
        ...
        PendingIntent startLW = PendingIntent.getActivity(this, 0, intent, getFlagIntent());
        intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
        return startLW;

    }

    private int getFlagIntent() {
//        return PendingIntent.FLAG_UPDATE_CURRENT;
    ...
    private void setAllowedVpnPackages(Builder builder) {
        ...
        if (!mProfile.isAllTraffics) {
            for (String pkg : mProfile.witAllowedApps) {
                try {
                    builder.addAllowedApplication(pkg);
                } catch (PackageManager.NameNotFoundException e) {
                    mProfile.mAllowedAppsVpn.remove(pkg);
...
//        TotalTraffic.calcTraffic(this, in, out, diffIn, diffOut);
        if (mDisplayBytecount) {
            ...
            sendMessage(
                    duration,
                    String.valueOf(lastPacketReceive),
                    byteIn, byteOut,
                    upload, download,
                    in, out
            );
    ...
    //sending message to main activity
    private void sendMessage(String duration,
        ...
        intent.putExtra("totalUpload", totalUpload);
        intent.putExtra("totalDownload", totalDownload);
        LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intent);
    }
